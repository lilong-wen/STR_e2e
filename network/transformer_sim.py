...
class SelfAttention(nn.Module):
    ...
    def __init__(self, embed_size, heads):
        ...
        assert (self.head_dim * heads == embed_size), "size mismatch"

        self.values = nn.Linear(self.head_dim, self.head_dim, bias=False)
        self.keys = nn.Linear(self.head_dim, self.head_dim, bias=False)
        self.queies = nn.Linear(self.head_dim, self.head_dim, bias=False)

        self.fc_out = nn.Liear(heads * self.head_dim, embed_size)
...
class Encoder(nn.Module):
    ...
    def __init__(self, src_vocab_size, embed_size, num_layers,
                 heads, devive, forward_expansion, dropout, max_length):
...
class Transformer(object):
    """Documentation for TransformerBlock
    ...
    def make_trg_mask(self, trg):
        ...
        trg_mask = torch.tril(torch.ones((trg_len, trg_len))).expend(
            N, 1, trg_len, trg_len
        )
