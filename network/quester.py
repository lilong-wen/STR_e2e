...
class QUESTER(nn.Module):
    ...
    def __init__(self, backbone, transformer, query_gen, num_classes, num_queries, aux_loss=False):
        ...
        # TODO class number should be changed
    ...
    def forward(self, samples: NestedTensor):
        ...
        """
        if isinstance(samples, target, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples)
        query_items = self.query_gen(target)

        hs = self.transformer(features, query_items, self.query_embed.weight, pos)[0]
